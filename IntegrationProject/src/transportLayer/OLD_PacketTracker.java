...
public class OLD_PacketTracker extends Observable implements NetworkLayer {
	...
	private LinkedHashMap<Short, OLD_TraceablePacket> pendingPackets = 
			new LinkedHashMap<Short, OLD_TraceablePacket>();
	...
	public OLD_PacketTracker(PacketRouter router, InetAddress address) {
		this.router = router;
	...
	public boolean sendData(byte[] dataToSend) {
		...
		if (!connectionAlive) {
			...
			OLD_TraceablePacket tp = new  OLD_TraceablePacket(trackNr, expectedNr, ControlFlag.ACK, dataToSend);
...
//		TestingTool.output(tp.toString());
		if (Arrays.equals(tp.getData(), new byte[0])) {
			...
			if (dataToSend != null) {
				...
				OLD_TraceablePacket packetToSend = new OLD_TraceablePacket(trackNr, expectedNr, flag, dataToSend);
		...
		for (int i = 0; i < pendingEntriesSet.size(); i++) {
			...
			if (e.getValue() <= time) {
				...
				if (timesSent.get(trackNrToResend) < MAX_SENT_TIMES) {
					OLD_TraceablePacket packetToResend = pendingPackets.get(trackNrToResend);
	...
	private boolean sendSynAck() {
		boolean couldSend = true;
		OLD_TraceablePacket synAck = new OLD_TraceablePacket(trackNr, expectedNr, ControlFlag.SYN_ACK, new byte[0]);
	...
	private boolean sendSetupPacket() {
		boolean couldSend = true;
		OLD_TraceablePacket synPacket = new OLD_TraceablePacket(trackNr, (short)0, ControlFlag.SYN, new byte[0]);
	...
	public void endConnection(boolean selfInitiated, OLD_TraceablePacket tp) {
		if (selfInitiated) {
			OLD_TraceablePacket finPacket = new OLD_TraceablePacket(trackNr, expectedNr, ControlFlag.FIN, new byte[0]);
			...
			OLD_TraceablePacket finAck = new OLD_TraceablePacket(trackNr, expectedNr, ControlFlag.FIN_ACK, new byte[0]);
